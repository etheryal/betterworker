...
use betterworker_sys::console_debug;
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if !this.awoken.get() {
            if this.closure.is_none() {
                ...
                // Then get that closure back and pass it to setTimeout so we can get woken up
                // later.
...
/// SAFETY: If, for whatever reason, the delay is dropped before the future is
/// ready JS will invoke a dropped future causing memory safety issues. To avoid
/// this we will just clean up the timeout if we drop the delay, cancelling the
/// timeout.
...
impl PinnedDrop for Delay {
    fn drop(self: Pin<&'_ mut Self>) {
        ...
        if let Some(id) = this.timeout_id {
            console_debug!("{:#?} has been dropped", &this.inner);
            let global: web_sys::WorkerGlobalScope = js_sys::global().unchecked_into();
