...
/// How long we'll attempt to connect to wrangler before we assume it's not
/// running.
...
pub fn expect_wrangler() {
    ...
    // Try to connect to wrangler's dev server, if it can't be reached assume the
    // user isn't running it.
...
#[allow(dead_code)]
pub fn delete(
    endpoint: &str, builder_fn: impl FnOnce(RequestBuilder) -> RequestBuilder,
) -> Response {
    expect_wrangler();
