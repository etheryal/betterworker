use betterworker_sandbox::{CreatePerson, Person};

use crate::util::{delete, expect_wrangler, get, post};
...
fn create_table() {
    let query = "CREATE TABLE IF NOT EXISTS people ( id INTEGER PRIMARY KEY, name TEXT NOT NULL, \
...
fn d1_update_prepare() {
    ...
    post(format!("d1/people/{}", update_person.id).as_str(), |rb| {
        rb.body(serde_json::to_string(&update_person).unwrap())
    });
...
fn create_person() -> Person {
    ...
    let response = post("d1/people", |rb| {
        rb.body(serde_json::to_string(&new_person).unwrap())
    });
