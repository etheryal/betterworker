...
pub async fn put(env: &Env) -> Result<Response<Body>, Error> {
    ...
    // Internally `.text()` calls `.bytes()` which calls `.stream()`, so most cases
    // are covered by just this check.
    ...
    // Miniflare behavior mismatch, in Miniflare an empty body will just return an
    // object without a body property. But in workerd it will only return an
    // object without a body property in the event that a condition failed
...
pub async fn put_multipart(env: &Env) -> Result<Response<Body>, Error> {
    ...
    // Now let's get the object again and ensure it consists of all three parts that
    // were uploaded.
