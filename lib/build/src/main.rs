...
fn copy_generated_code_to_worker_dir() -> Result<()> {
    ...
    // wasm-bindgen supports adding arbitrary JavaScript for a library, so we need
    // to move that as well. https://rustwasm.github.io/wasm-bindgen/reference/js-snippets.html
...
// Replaces the wasm import with an import that instantiates the WASM modules
// itself.
...
// After bundling there's no reason why we'd want to upload our now un-used
// JavaScript so we'll delete it.
